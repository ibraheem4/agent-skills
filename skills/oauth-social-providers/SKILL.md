...
description: Use when wiring Google or Microsoft social sign-in for a WorkOS AuthKit environment — registering the upstream Entra app or Google OAuth client, capturing a client secret that is shown exactly once, or saving credentials onto a credential record. Carries the ordering that cannot be replayed, the two provider gates that reject sign-ins for reasons unrelated to your credentials, and how to prove a provider is wired without signing in.
---
...
Three of these rules cost a credential each to learn. Both providers display a client secret
once and never again, so the expensive mistakes are all ordering mistakes. The other half of
the skill is the opposite problem: sign-ins that fail, or appear to work, for reasons that
have nothing to do with the credentials you just configured.
...
## 1. Ordering — the callback slug does not exist yet

The callback URI carries a random per-record slug, so it cannot be known before the record
exists:

1. Create the credential record in the dashboard. Selecting "your app's credentials" creates
   it instantly — no Save. There is no API to create one.
2. Read `redirectUri` off the API. Never reconstruct it.
...
Consent screen: app name, audience **External**, support and contact email. Then a client of
type **Web application** with the callback as its one redirect URI. Verify that URI
character-exactly before creating — an accessibility tree does not expose input values, so
read it deliberately rather than trusting a truncated view where the tail looks right.

## 5. Two gates that are not your credentials
...
works. It still matters — the consent screen is theirs and you control none of it — but it is
not an outage, and it is not a reason to declare an incident.
...
After saving credentials, confirm over the API that the record's state moved to valid and
that its redacted secret matches the tail of what you banked.
